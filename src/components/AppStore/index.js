...
class AppStore extends Component {
  state = {searchInput: '', activeApp: tabsList[0].tabId}
  ...
  tabItem = tabId => {
    this.setState({activeApp: tabId})
  }

  getFilterApp = () => {
    const {searchInput, activeApp} = this.state
    const filterApps = appsList.filter(
      each =>
        each.category === activeApp &&
          ...
          <ul className="tab-container">
            {tabsList.map(eachTab => (
              <TabItem
                ...
                isActive={eachTab.tabId === activeApp}
              />
            ))}
          </ul>

          <ul className="app-main-item-container">
